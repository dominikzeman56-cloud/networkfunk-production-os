#!/usr/bin/env node
/**
 * NPOS API Server - Fallback Version (for testing without SQLite)
 * Uses JSON file-based storage instead of SQLite
 */
...
const DB_PATH = resolvePath(cfg.paths.database || join(cfg.paths.data, 'npos-fallback'));
...
// Auto-migrate JSON → fallback storage on first boot (no-op if already migrated)
try {
  const mig = layer.migrateFromJson({ force: false });
  if (mig.skipped) {
    console.log(`[NPOS] Fallback storage already migrated (${mig.at})`);
  } else {
    console.log(`[NPOS] Migrated JSON → Fallback storage:`, mig.report);
  }
} catch (err) {
...
server.listen(PORT, HOST === 'localhost' ? undefined : HOST, () => {
  console.log(`\n  NPOS API Server (Phase 1 - Fallback) running at http://${HOST}:${PORT}`);
