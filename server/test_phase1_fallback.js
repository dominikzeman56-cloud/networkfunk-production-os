#!/usr/bin/env node
/**
 * NPOS Phase 1 Fallback Test Suite
 * Tests all Phase 1 API endpoints and functionality using the fallback JSON implementation
 */
...
const TEST_DATA_DIR = join(DATA_DIR, 'test-fallback');
...
class NposTester {
  ...
  async setupTestEnvironment() {
    ...
    const files = ['session.json', 'projects.json', 'build-sheets.json', 'logs.json', 'presets.json', 'knowledge-index.json'];
    files.forEach(file => {
    ...
    // Copy existing session data if it exists
    const existingSessionPath = join(DATA_DIR, 'session.json');
    if (existsSync(existingSessionPath)) {
      const sessionData = readFileSync(existingSessionPath, 'utf-8');
      writeFileSync(join(TEST_DATA_DIR, 'session.json'), sessionData);
    }
  }

  async startServer() {
    return new Promise((resolve, reject) => {
      console.log('Starting NPOS fallback server...');
      this.serverProcess = spawn('node', ['--watch', 'api-fallback.js'], {
        cwd: __dirname,
        stdio: ['ignore', 'pipe', 'pipe'],
        shell: true,
        env: {
          ...process.env,
          NPOS_DATA_DIR: TEST_DATA_DIR
        }
      });

      this.serverProcess.stdout.on('data', (data) => {
        const output = data.toString();
        if (output.includes('NPOS API Server (Phase 1 - Fallback) running')) {
  ...
  async stopServer() {
    if (this.serverProcess) {
      console.log('Stopping NPOS fallback server...');
  ...
  async testMigration() {
    ...
    try {
      ...
      if (response.ok) {
        console.log('✅ POST /api/migrate working');
        console.log(`   Migrated ${data.migratedFiles} files`);
        ...
        console.error('❌ POST /api/migrate failed:', data);
        return false;
      }
    } catch (err) {
      console.error('❌ POST /api/migrate error:', err.message);
      return false;
    }
  }
  ...
  async runAllTests() {
    console.log('🚀 Starting NPOS Phase 1 Fallback Test Suite\n');
    ...
    try {
      ...
      if (passed === total) {
        console.log('🎉 All Phase 1 fallback tests passed!');
        return true;
      } else {
